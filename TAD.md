...
- 系统设计与实现：设计并实现基于深度学习的实时交通事故检测与风险评估系统，该系统能够实时处理道路监控视频，检测交通事故，并评估事故风险。
- 算法集成应用：集成YOLOv10目标检测算法和BoT-SORT轨迹跟踪算法，实现对车辆的精确检测和轨迹跟踪。

- 视角转换方法：提出一种新的视角转换方法，将车辆在图像中的像素坐标映射为真实世界坐标，以提高事故检测的准确性。

- 风险评分计算：创新性地提出基于车辆运动特征的风险评分计算方法，包括速度变化、轨迹角度变化、重叠度等因素的综合评估。
- 系统优化与实验验证：通过大量实验验证系统性能，不断优化算法和系统参数，以适应不同的交通环境和场景。
...
graph TD
    ...
    F--系统初始化-->G[(源数据)]
    model-->G
    G--Yolov10车辆检测&Bot-Sort车辆追踪-->H[(带有车辆特征的数据)]
    H--高斯滤波&视角转换-->I[(待计算数据)]
    I--提取-->I1[(可视化数据)]
    I-->J[/物理特征提取\]
    J-->J1{{角度}}
...
​	ARMS采用模块化设计，前端控制，由数据输入、核心处理和数据输出三大模块组成，以实现功能的扩展和解耦。系统的主要目标是通过视频分析技术，实现交通事故的实时检测与预测。
...
​	在yolo中，为了进行检测，通常将输入图像尺寸调整为640×640像素。
...
​	在一般的映射逻辑中，检测框的中心点$$K$$被用作投影变换的依据。然而，由于车辆是三维立体的物体，而投影的检测框仅是二维平面上的近似，因此在映射过程中会产生误差，丢失关于汽车的深度数据。
...
​	另一种方法则是作者使用的数学方法，可以简略估计出汽车底盘的近似代表点：通过检测框的几何关系来估计底盘中心点的位置，减少误差。通常情况下，道路上安置的摄像头其俯视角的平均角度为30°，可以获得更详细的信息和较好的视野。这时我们可以认为，汽车在摄像头前行驶的过程中与摄像头所成的平均角度为30°，为了最小化误差，我们以该角度作为基准用于估计底盘投影在检测框中代表的实际位置(图4)。
...
​	为了全面评估车辆的风险情况，ARMS综合考虑了速度、波动、角度变化、曲率以及检测框重叠度等因素，并为每个因素分别计算评分，最终通过最大风险因子法计算得到综合风险评分。下述的计算方式是作者通过对于本问题的分析研究所提出的一种合理的映射方式：
