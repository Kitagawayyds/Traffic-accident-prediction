...
vehicle_length = 20  # 长(汽车)
vehicle_width = 10  # 宽(汽车)
...
if show_transform:
    out_t = cv2.VideoWriter(transform_path, fourcc, target_fps, (width_t, height_t))
...
for _ in tqdm(range(total_frames), desc="Processing"):
    ...
    if show_transform:
        frame_t = np.ones((height_t, width_t, 3), dtype=np.uint8) * 255
