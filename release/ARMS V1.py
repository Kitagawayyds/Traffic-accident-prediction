...
def main():
    ...
    **Made by Linhan Song😘**
    ...
    with st.sidebar.form("parameters"):
        ...
        vehicle_model = st.file_uploader("Select Vehicle Model", type=["pt"])
        ...
        vehicle_model = vehicle_model.name if vehicle_model else "yolov10n.pt"
        video_path = video_path.name if video_path else "test2.mp4"
        transform_file_path = transform_file_path.name if transform_file_path else "transform.txt"
        spec_file_path = spec_file_path.name if spec_file_path else "spec.txt"

        target_fps = st.slider("Target FPS", min_value=1, max_value=30, value=20)
    ...
    if submit_button:
                ...
                max_overlap = max(
                    (
                        bb_overlap(
                            ...
                            current_angle,
                            calculate_angle(effective_track[other_id])[1] if calculate_speed(effective_track[other_id])[
                                                                                 0] >= resting_threshold else last_known_angles.get(
                                other_id, 0),
                            class_name[track_id],
                            class_name[other_id]
                        )
        ...
        if mode == "Video":
            ...
            frame_generator = iter(int, 10000)  # 无限迭代器
        ...
        for _ in frame_generator:
            ...
            cv2.putText(annotated_frame, f"ARMS V1", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (71, 210, 160), 2,
                        cv2.LINE_AA)
            ...
            # 事故检测
            acc_detected, acc_vehicles = detect_accidents(track_history, boxes, class_names)
        ...
        cap.release()
        if capture_track:
            st.text(f"Capture Track: {catch_track}")
        if save:
            out.release()
        if show_transform:
            out_t.release()
