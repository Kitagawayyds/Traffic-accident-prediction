...
def process_and_plot_tracks(track, view_transformer, sigma):
    # Smooth the original track
    smoothed_track = smooth_track(track, sigma)

    # Transform the smoothed track
    transformed_smoothed_track = view_transformer.transform_points(np.array(smoothed_track))

    # Smooth the transformed track
    smoothed_transformed_track = smooth_track(transformed_smoothed_track, sigma)

    # Transform the original track
    transformed_track = view_transformer.transform_points(np.array(track))

    # Smooth the transformed track
    smoothed_transformed_track_2 = smooth_track(transformed_track, sigma)

    # Plot all tracks in separate subplots
    fig, axs = plt.subplots(2, 2, figsize=(18, 12))

    # Plot original track
    axs[0, 0].plot([p[0] for p in track], [p[1] for p in track], 'o-', label='Original Track', color='blue')
    ...
    # Plot smoothed track
    axs[0, 1].plot([p[0] for p in smoothed_track], [p[1] for p in smoothed_track], 'x-', label='Smoothed Track',
                   color='red')
    axs[0, 1].set_title('Smoothed Track')
    ...
    # Plot transformed and smoothed track
    axs[1, 0].plot([p[0] for p in transformed_track], [p[1] for p in transformed_track], 's--',
                   label='Transformed Track', color='cyan')
    axs[1, 0].plot([p[0] for p in smoothed_transformed_track], [p[1] for p in smoothed_transformed_track], 'd--',
                   label='Smoothed and Transformed Track', color='green')
    axs[1, 0].set_title('Transformed and Smoothed Tracks')
    ...
    # Plot transformed and then smoothed track
    axs[1, 1].plot([p[0] for p in transformed_track], [p[1] for p in transformed_track], 's--',
                   label='Transformed Track', color='cyan')
    axs[1, 1].plot([p[0] for p in smoothed_transformed_track_2], [p[1] for p in smoothed_transformed_track_2], 'p--',
                   label='Transformed and Smoothed Track', color='orange')
    axs[1, 1].set_title('Transformed and Smoothed Tracks')
...
# Process and plot tracks
process_and_plot_tracks(track, view_transformer, sigma=2)
