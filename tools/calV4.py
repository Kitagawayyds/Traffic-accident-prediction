...
def curvature_risk(curvature, speed, curvature_threshold=1):
...
# Create a range of curvature values from 0 to 2 with a fixed speed of 50
curvature_values = np.linspace(0, 2, 500)
curvature_risk_scores = [curvature_risk(curvature, 50) for curvature in curvature_values]

# Plotting the risk scores
plt.figure(figsize=(24, 6))  # Adjusted figure size for additional subplot
...
# Plot curvature risk score subplot
plt.subplot(1, 5, 5)
plt.plot(curvature_values, curvature_risk_scores, label='Curvature Risk Score', color='purple')
plt.xlabel('Curvature')
plt.ylabel('Risk Score')
plt.title('Curvature to Risk Score Mapping')
plt.grid(True)
plt.legend()
