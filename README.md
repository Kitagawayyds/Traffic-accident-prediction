...
## ARMS V1
此版本为正式版，为程序添加了前端，以便更好地操作，添加了事故检测曲线，用于指示视频上哪里发生了交通事故。同时添加了实时检测模式，更符合实际应用中的情况。详情请看代码。
运行方式：
...
![ 2024-08-15 162342.png](https://s2.loli.net/2024/08/15/eK6D3jshga1u59r.png)

## ARMS V1.0.1
- 添加事故帧保存功能，将事故帧的信息保存在本地，便于分析。
- 优化了detect的部分逻辑，减少计算量。

至此整个程序已趋近于完善，下面的工作主要以查找bug和逻辑错误，完整性检查，封包解耦，以及报错处理，为主。

## ARMS V1.0.2
- 修复了一些bug

## ARMS V1.0.3
- 修改了检测逻辑，现在只会针对在目标区域内的目标进行车祸检测，进一步减少了误报的可能性以及准确率。
- 通过数据调查以及几何计算进一步优化了代表点获取的计算逻辑，更加得准确了。

## ARMS V1.0.4
- 针对于目标重识别的问题（即一个目标被检测算法检测两次，出现两个box），现在使用两个检测框代表点之间的距离是否正常来判断是否是重识别，若重识别则不进行重叠度的计算。
- 优化了重叠度计算的时间复杂度

## ARMS V1.0.5
- 使用trt进行模型推理提高推理效率，优化目标检测参数
